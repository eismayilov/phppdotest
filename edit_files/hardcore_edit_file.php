...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $pdo->prepare('SELECT * FROM hardcore_pets WHERE id = ?');
    $stmt->execute([$id]);
    $hardcores = $stmt->fetch(PDO::FETCH_ASSOC);
}
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $image = $hardcores['image'];
    }

    // Обновление данных продукта в базе данных
    $stmt = $pdo->prepare('UPDATE hardcore_pets SET name = ?, gem_value = ?, demand = ?, value_change = ?, image = ? WHERE id = ?');
    $stmt->execute([$name, $gem_value, $demand, $value_change, $image, $id]);
    header('Location: ../hardcore_pets.php');
    exit();
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <form class="update-form" method="POST" action="" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?= $hardcores['id'] ?>">

        <div class="update-image">
            <img src="../uploads/<?= $hardcores['image'] ?>" alt="<?= $hardcores['name'] ?>">
            <label for="image">Выбор фото:</label>
            <input type="file" id="image" name="image">
        </div>
        <div class="form-group">
            <label for="name">Имя питомца:</label>
            <input type="text" id="name" name="name" value="<?= $hardcores['name'] ?>">
        </div>

        <div class="form-group">
            <label for="gem_value">Цена в гемах :</label>
            <input type="text" id="gem_value" name="gem_value" value="<?= $hardcores['gem_value'] ?>">
        </div>

        <div class="form-group">
            <label for="demand">Спрос:</label>
            <input type="text" id="demand" name="demand" value="<?= $hardcores['demand'] ?>">
        </div>

        <div class="form-group">
            <label for="value_change">Цена изменилась:</label>
            <input type="text" id="value_change" name="value_change" value="<?= $hardcores['value_change'] ?>">
        </div>
